    ...
    <div class="container my-5" style="background: rgba(255,255,255,0.95); padding: 30px; border-radius: 10px;">
        ...
        @if (empty($cart))
            <div class="alert alert-info text-center">
                ...
                <a href="{{ url('/productosPub') }}" class="btn btn-primary">Volver a la tienda</a>
            </div>
        @else
                            ...
                            <tr>
                                <td>
                                    @if (str_contains($detalles['imagen'], 'storage/'))
                                        <img src="{{ asset($detalles['imagen']) }}" width="50" height="50"
                                            class="img-thumbnail me-2">
                                    @else
                                        <img src="{{ asset('img/' . $detalles['imagen']) }}" width="50"
                                            height="50" class="img-thumbnail me-2">
                                    @endif
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center align-items-center gap-1">
                                        <form action="{{ route('cart.update', $id) }}" method="POST" class="m-0">
                                            ...
                                            <button type="submit"
                                                class="btn btn-sm btn-outline-secondary py-1 px-2">-</button>
                                        </form>

                                        <span class="fw-bold px-2"
                                            style="min-width: 30px;">{{ $detalles['cantidad'] }}</span>
